...
# (symbol, company name, GICS sector) -- alphabetical by symbol.
SP500_CONSTITUENTS: tuple[tuple[str, str, str], ...] = (
...
# A small curated set of common benchmark indices/proxies, distinct from the
# equity universe above (indices aren't equities and some use "^"-prefixed
# Yahoo Finance symbols) -- also constrained-selection-only in the UI.
...
_TICKER_INDEX: dict[str, str] = {symbol: name for symbol, name, _ in SP500_CONSTITUENTS}
_BENCHMARK_INDEX: dict[str, str] = dict(BENCHMARKS)


def is_valid_ticker(symbol: str) -> bool:
    return symbol.strip().upper() in _TICKER_INDEX


def is_valid_benchmark(symbol: str) -> bool:
    return symbol.strip().upper() in _BENCHMARK_INDEX


def ticker_name(symbol: str) -> str | None:
    return _TICKER_INDEX.get(symbol.strip().upper())


def benchmark_name(symbol: str) -> str | None:
    return _BENCHMARK_INDEX.get(symbol.strip().upper())
