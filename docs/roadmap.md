...
- [ ] Phase 2 — Quant Core: Factor & Optimization Models (`quant-analyst`): Build CAPM beta estimation, Fama-French 3-factor (5-factor stretch) regression with proper statistical diagnostics, and Markowitz mean-variance efficient-frontier optimization against the user's actual live-data holdings. Output: a structured, reusable model module (not a one-off notebook) serving beta, factor loadings, and frontier coordinates for any given holdings set.
- [ ] Phase 3 — Explainable Attribution & Visualization Layer (`business-intelligence`): Turn Phase 2's model outputs into visual, explainable attribution — factor exposure breakdown, efficient-frontier chart with the current portfolio plotted against it, and a return/risk attribution view showing how much traces to each factor vs. idiosyncratic alpha. Output: a dashboard wired to live model outputs, not static mockups.
