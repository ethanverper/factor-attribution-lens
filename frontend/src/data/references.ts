...
//
// `legend` restructured per decision 0022 (Phase 10n, `brand-creative`) from a
// single middot-joined prose line into a real definition list; the Markowitz
// and ticker-universe cards' dense closing notes decomposed into
// lead/bullets/callout. Every other card's `note` is left as a bare string —
// audited and found already fine.
...
export interface ReferenceCard {
  ...
  formulas: Formula[]
  legend: LegendEntry[]
  legendCaption?: string // trailing note that isn't a term definition (Fama-French card only)
  legendNote?: string // for the one card whose "legend" documents a policy, not variables (ticker/benchmark)
  note: string
  noteBullets?: string[] // Markowitz + ticker cards only
  noteCallout?: { variant: "note" | "caveat"; label?: string; text: string } // Markowitz + ticker cards only
  source: string
}
...
export const REFERENCE_CARDS: ReferenceCard[] = [
  {
    ...
    legend: [
      { term: "R<sub>p,t</sub>", def: "portfolio return at t" },
      { term: "R<sub>f,t</sub>", def: "risk-free rate (Ken French Data Library)" },
      { term: "R<sub>m,t</sub>", def: "benchmark return" },
      { term: "&beta;", def: "market beta" },
      { term: "&alpha;", def: "intercept (excess return unexplained by the market)" },
    ],
  ...
  },
  {
    ...
    legend: [
      { term: "SMB", def: "Small Minus Big (size)" },
      { term: "HML", def: "High Minus Low (value, book-to-market)" },
      { term: "RMW", def: "Robust Minus Weak (profitability)" },
      { term: "CMA", def: "Conservative Minus Aggressive (investment)" },
    ],
    legendCaption: "Each is itself a spread portfolio return, not a raw price series.",
  ...
  },
  {
    ...
    legend: [
      { term: "n", def: "number of aligned observations in the regression sample" },
      { term: "L", def: "number of autocorrelation lags included in the HAC covariance estimate" },
    ],
  ...
  },
  {
    ...
    legend: [
      { term: "w", def: "holding weight vector" },
      { term: "&mu;", def: "vector of expected (annualized) holding returns" },
      { term: "&Sigma;", def: "annualized covariance matrix of holding returns" },
      { term: "R<sub>f</sub>", def: "risk-free rate" },
    ],
  ...
  },
  {
    ...
    legend: [
      { term: "&beta;<sub>i</sub>", def: "the fitted loading on factor i" },
      { term: "mean(factor<sub>i</sub>)", def: "that factor's own realized mean return over the same aligned window" },
    ],
